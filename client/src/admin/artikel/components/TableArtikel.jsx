...
const TableArtikel = ({ data, onDelete }) => {
                    ...
                    <table className="w-full text-sm text-left text-gray-500">
                        <thead className="text-sm text-white font-primary font-medium bg-sky-700">
                            <tr>
                                ...
                                <th scope="col" className="px-8 py-6 ">Author</th>
                            ...
                            {data.length === 0 ? (
                                ...
                                renderData().map((artikel, index) => (
                                    <tr className="border border-gray-200" key={index}>
                                        ...
                                        <td className="font-primary text-xs font-regular text-gray-800 px-8 py-4 max-w-xs truncate">{artikel.judul}</td>
                                        <td className="font-primary text-xs font-regular text-gray-800 px-8 py-4 max-w-xs truncate">{artikel.author}</td>
                                        <td className="font-primary text-xs font-regular text-gray-800 px-8 py-4 max-w-xs truncate">
                                            {moment(artikel.tanggal).format('DD MMMM YYYY')} </td>
                                        <td className="px-12 py-3 flex items-center justify-end space-x-4">
                                            <Link to={`/data-artikel/edit/${artikel.id}`}>
                                                <Button
