...
function EditArtikel() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3001/artikel/${id}`)
      .then((res) => {
  ...
  const handleUpdate = (e) => {
    ...
    axios
      .put(`http://localhost:3001/artikel/update/${id}`, formData)
      .then((res) => {
        console.log("Update success:", res);
        navigate("/data-artikel", {
          state: { alertMessage: `Berhasil mengubah ${values.judul}` },
        });
      })
      .catch((err) => {
        console.error(
  ...
  return (
    <Layout>
      <div className="py-8 px-4 md:mx-52 max-w-xl  lg:py-13">
        <h2 className="mb-5 text-2xl font-semibold font-primary text-gray-900">
          Ubah Artikel
        </h2>
