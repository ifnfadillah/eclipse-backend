  ...
  useEffect(() => {
    const fetchOtherWebinars = async () => {
      try {
        ...
        const shuffledWebinars = otherWebinars
      ...
      <div className="w-full mt-5 sm:px-8 py-10 flex flex-col items-center gap-12">
        ...
        <div className="flex flex-col md:flex-row items-center px-8">
          ...
          <div className="w-full md:w-1/3 mb-4 md:mb-0">
            <img
              src={`http://localhost:3001/uploads/${webinar.foto}`}
              alt={webinar.judul}
          ...
          <div className="w-full md:w-2/3 md:px-24 space-y-4 md:space-y-8 font-primary">
            ...
            <h3 className="text-sm font-medium">
              Penyelenggara:{" "}
              <span className="text-sm font-normal">{webinar.narasumber}</span>
            </h3>
            {/* Tanggal webinar */}
            <h3 className="text-sm font-medium">
              Tanggal:{" "}
              <span className="text-sm font-normal">
                {moment(webinar.tanggal).format("DD MMMM YYYY")}
              </span>
            </h3>
            {/* Waktu webinar */}
            <h3 className="text-sm font-medium">
              Pukul:{" "}
              <span className="text-sm font-normal">
                {moment(webinar.waktu, "HH:mm").format("HH:mm")}
              </span>
            </h3>
            {/* Harga webinar */}
            <h3 className="text-sm font-medium">
